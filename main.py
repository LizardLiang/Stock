...
class stock_thread(threading.Thread):
    def __init__(self, queue, ip, serial, p_queue):
        # initial class
        threading.Thread.__init__(self)
        self.queue = queue
        self.p_q = p_queue
    ...
    def run(self):
        ...
        while self.queue.qsize() > 0:
            ...
            while self.year >= 2010:
                ...
                while len(m_list) > 0:
                    m = m_list.pop()
                    is_exist = sql.check_date(id, self.year, m)
                    # Don't need future data
                    if (self.year == 2020 and m > 11) or (is_exist == 1):
                        pass
                    else:
...
def test_ip():
    ...
    while not id_queue.empty():
        ...
        for i in range(40):
            ...
            stock_th.append(stock_thread(id_queue, "", i, p_queue))
